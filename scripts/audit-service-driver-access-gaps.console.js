 ...
 *
 * Retorna:
 * - todos os funcionarios sem `cr40f_emailmicrosoft`;
 * - servicos de 45 dias para tras ate 10.000.000 dias para frente
 *   com motorista designado, mas sem compartilhamento direto de leitura
 *   para o `systemuser` correspondente ao motorista;
 * - relacionados do servico sem acesso: manutencao vinculada, servicos por
 *   passageiro, passageiros e solicitante.
 *
 * Resultado completo:
 * - window.ServiceDriverAccessAuditLastResult
 */
(async () => {
  ...
  const ACTION_PREVIEW_LIMIT = 300;
  ...
  const employeesWithoutMicrosoftEmail = [];
  const missingAccess = [];
  ...
  const summary = {
    ...
    endDateUsed: "",
    employeesWithoutMicrosoftEmail: 0,
    scannedServices: 0,
    scannedTargets: 0,
    resolvedDrivers: 0,
    targetsMissingDriverReadAccess: 0,
    skippedNoDriver: 0,
    skippedNoEmail: 0,
    skippedNoUser: 0,
  ...
  function log(...args) {
    console.log("[service-driver-access-audit]", ...args);
  }
  ...
  const employees = await listAll(
    `/${EMPLOYEE_ENTITY_SET}?$select=${EMPLOYEE_ID},${EMPLOYEE_NAME},${EMPLOYEE_EMAIL},${EMPLOYEE_FALLBACK_EMAIL}&$filter=${EMPLOYEE_EMAIL} eq null&$orderby=${EMPLOYEE_NAME} asc`
  );
  for (const employee of employees) {
    employeesWithoutMicrosoftEmail.push({
      employeeId: cleanGuid(employee[EMPLOYEE_ID]),
      employeeName: String(employee[EMPLOYEE_NAME] || "").trim(),
      emailMicrosoft: "",
      emailBetinhos: String(employee[EMPLOYEE_FALLBACK_EMAIL] || "").trim()
    });
  }
  summary.employeesWithoutMicrosoftEmail = employeesWithoutMicrosoftEmail.length;
  ...
  summary.targetsMissingDriverReadAccess = missingAccess.length;

  const result = {
    summary,
    employeesWithoutMicrosoftEmail,
    missingAccess,
    identityIssues,
    nextGrantPreview: missingAccess.slice(0, ACTION_PREVIEW_LIMIT)
  };
  window.ServiceDriverAccessAuditLastResult = result;

  log(`Ambiente: ${baseUrl}`);
  console.table(summary);
  console.table(employeesWithoutMicrosoftEmail);
  console.table(missingAccess.slice(0, ACTION_PREVIEW_LIMIT));
  if (identityIssues.length) console.table(identityIssues.slice(0, ACTION_PREVIEW_LIMIT));
  log("Resultado completo em window.ServiceDriverAccessAuditLastResult");
})();
